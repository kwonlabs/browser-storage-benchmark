...
export const cookieBenchmark: BenchmarkUnit = {
  ...
  name: "Cookie",
  description:
    "Traditional browser cookies. Very limited size (4KB) and can impact network performance as they are sent with every request.",
  icon: "🍪",
  category: "low",
    ...
    return {
      insert: () => {
        if (payloads.original.length > COOKIE_LIMIT) {
          throw new Error(
            `[Cookie] Size limit exceeded (${payloads.original.length} bytes). Cookies are limited to ~4KB.`
          );
        }
      ...
      update: () => {
        if (payloads.modified.length > COOKIE_LIMIT) {
          throw new Error(
            `[Cookie] Size limit exceeded (${payloads.modified.length} bytes). Cookies are limited to ~4KB.`
          );
        }
