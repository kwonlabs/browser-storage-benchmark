...
export const lzmaBenchmark: BenchmarkUnit = {
  ...
  name: "LZMA",
  description:
    "High-compression algorithm (Lempel-Ziv-Markov) via JS implementation. Slow but very efficient.",
  icon: "📦",
  ...
  run: (
    ...
    return {
      setup: async () => {
        ...
        // @ts-ignore
        const { default: lzmaContent } =
          await import("lzma/src/lzma_worker-min.js?raw");
      ...
      compress: async () => {
        return new Promise((resolve) => {
          lzmaLib.compress(payload, 1, (result: any) =>
