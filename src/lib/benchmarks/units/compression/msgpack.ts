...
export const msgpackBenchmark: BenchmarkUnit = {
    id: 'msgpack',
    name: 'MessagePack',
    description: 'An efficient binary serialization format. It lets you exchange data among multiple languages like JSON, but it is faster and smaller.',
    icon: '📦',
    ...
    run: (_sizeName: string, _sizeValue: number, _payloads: { original: any; modified: any }): CompressionStepDefinitions => {
        ...
        try {
            if (typeof _payloads.original === 'string' && (_payloads.original.startsWith('{') || _payloads.original.startsWith('['))) {
