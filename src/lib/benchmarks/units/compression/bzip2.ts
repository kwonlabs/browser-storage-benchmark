...
export const bzip2Benchmark: BenchmarkUnit = {
  ...
  name: "Bzip2",
  description:
    "High-quality data compressor (Burrows-Wheeler) via WebAssembly. Excellent for text.",
  icon: "🗜️",
  ...
  run: (
    ...
    const handleError = (
      ...
      throw new Error(
        ERROR_MESSAGES[returnValue] || `error code: ${returnValue}`
      );
    };
    ...
    return {
      setup: async () => {
        ...
        const loadBZip2WASM = (await import("bzip2-wasm/bzip2-1.0.8/bzip2.mjs"))
          .default;
        wasmModule = await loadBZip2WASM({
          locateFile: () => "/wasm/bzip2.wasm",
        });
      },
      compress: async () => {
