...
export async function getAllSessions(type: 'storage' | 'compression'): Promise<BenchmarkSession[]> {
    const db = await openDB();
    return new Promise((resolve, reject) => {
        const storeName = getStoreName(type);
...
export async function getSession(type: 'storage' | 'compression', id: number): Promise<BenchmarkSession | undefined> {
    const db = await openDB();
    return new Promise((resolve, reject) => {
        const storeName = getStoreName(type);
        const tx = db.transaction(storeName, 'readonly');
...
export async function deleteSession(type: 'storage' | 'compression', id: number): Promise<void> {
    const db = await openDB();
    return new Promise((resolve, reject) => {
        const storeName = getStoreName(type);
        const tx = db.transaction(storeName, 'readwrite');
...
export async function clearAllSessions(type: 'storage' | 'compression'): Promise<void> {
    const db = await openDB();
    return new Promise((resolve, reject) => {
        const storeName = getStoreName(type);
        const tx = db.transaction(storeName, 'readwrite');
