...
export interface RunnerCallbacks {
    ...
    onLog: (message: string, type?: 'system' | 'success' | 'error', level?: 'debug' | 'info' | 'warn' | 'error') => void;
...
let testQueue: { task: TaskDef; payloadTypes: PayloadType[], selectedUnitIds: string[] | null }[] = [];
...
function initWorkers() {
    ...
    function handleWorkerError(worker: 'native' | 'wrapper' | 'compression') {
        return (e: ErrorEvent) => {
            ...
            callbacks?.onLog(`⚠ ${worker} worker error: ${e.message}`, 'error', 'error');
            completedUnits++;
            isProcessing = false;
            runNext();
        };
...
// ──────────────────────────────────────────────
function recordResult(unitId: string, sizeName: string, payloadType: string, result: any) {
    ...
    if (unit.category === 'compression') {
        ...
        if (!latestData[storageCategory][sizeName]) latestData[storageCategory][sizeName] = {};
        if (!latestData[storageCategory][sizeName][payloadType]) latestData[storageCategory][sizeName][payloadType] = {};
        (latestData[storageCategory][sizeName][payloadType] as Record<string, any>)[unit.name] = result;
    }
}

function handleWorkerMessage(e: MessageEvent, category: 'high' | 'compression') {
    if (!isRunning) return;
    const { type, unitId, sizeName, payloadType, result, step, iteration, total } = e.data;
    ...
    if (type === 'iteration_progress') {
        if (timeoutHandle) {
            ...
            timeoutHandle = setTimeout(() => {
                if (!isRunning) return;
                const unit = getBenchmarkById(unitId);
                console.warn(`[Runner] Timeout for ${unit?.name || unitId} (${unitId})`);
                callbacks?.onLog(`⚠ Timeout: ${unit?.name || unitId} skipped`, 'error', 'warn');
                initWorkers();
                completedUnits++;
                isProcessing = false;
                runNext();
        ...
        const unit = getBenchmarkById(unitId);
        if (unit) {
            const durStr = e.data.duration !== undefined ? ` (${e.data.duration.toFixed(2)}ms)` : '';
            callbacks?.onLog(`[${unit.name}] ${step} iteration ${iteration}/${total} finished${durStr}`, 'system', 'debug');
            const percent = Math.round((completedUnits / (totalUnits || 1)) * 100);
            callbacks?.onProgress(percent, `Running: ${unit.name} (${sizeName.toUpperCase()}, ${payloadType}) - ${step} ${iteration}/${total}...`);
        }
        return;
    }
    ...
    if (unit) {
        if (e.data.error) {
            callbacks?.onLog(`[${unit.name}] Worker Execution Error: ${e.data.error}`, 'error', 'error');
        }
        ...
        callbacks?.onLog(`Done ${catLabel} for ${unit.id} ${sizeName.toUpperCase()}/${pType} (${duration}ms)`, 'success', 'info');

        if (result?.driverInfo) {
            callbacks?.onLog(`[${unit.name}] Selected Runtime Driver: ${result.driverInfo}`, 'system', 'info');
        }
...
async function runMainThreadUnit(unitId: string, sizeName: string, sizeValue: number, payloadType: PayloadType) {
    ...
    try {
        ...
        if (iterations > 1) {
            callbacks?.onLog(`[${unit.name}] Running ${iterations.toLocaleString()} iterations for precision...`, 'system', 'debug');
        }

        if (unit.category === 'compression') {
            const result = await runCompressionLifecycle(sizeValue, steps as CompressionStepDefinitions, original, unit.name, iterations);
            if (!isRunning) return;
            recordResult(unitId, sizeName, payloadType, result);
        } else {
            const result = await runStorageLifecycle(sizeValue, steps as StorageStepDefinitions, { original, modified }, unit.name, iterations);
            if (!isRunning) return;
            recordResult(unitId, sizeName, payloadType, result);
        ...
        callbacks?.onLog(`[${unit.name}] Main Thread Error: ${err.message || err}`, 'error', 'error');
    }
}
...
export async function runNext() {
    ...
    if (unitQueue.length === 0) {
        ...
        payloadTypes.forEach(pt => {
            benchmarks.forEach(b => {
                ...
                if (['sessionstorage', 'localstorage', 'store2'].includes(b.id) && task.sizeValue >= 4 * 1024 * 1024) return;
    ...
    if (unit.isSupported) {
        try {
            ...
            const reason = typeof result === 'object' ? result.reason : 'Not supported in this environment';
            ...
            callbacks?.onLog(`⚠ Support check failed for ${unit.name}: ${e.message}`, 'error', 'error');
            completedUnits++;
            isProcessing = false;
            runNext();
            return;
    ...
    const catLabel = unit.category === 'compression' ? 'compression' : 'storage';
    const pType = payloadType.toUpperCase();
    callbacks?.onProgress(percent, `Running: ${unit.name} (${sizeName.toUpperCase()}, ${payloadType})...`);
    callbacks?.onLog(`Start ${catLabel} for ${unit.name} ${sizeName.toUpperCase()}/${pType}`, 'system', 'info');
    ...
    if (unit.runType === 'worker.async') {
        ...
        if (worker) {
            worker.postMessage({ unitId, sizeName, sizeValue, payloadType, iterations });
        } else {
        ...
        const unitPromise = runMainThreadUnit(unitId, sizeName, sizeValue, payloadType);
        ...
        try {
            ...
            if (err.message === 'TIMEOUT') {
                ...
                callbacks?.onLog(`[${unit.name}] Execution Error: ${err.message || err}`, 'error', 'error');
            }
        } finally {
            completedUnits++;
            isProcessing = false;
...
export async function startBenchmark(tasks: TaskDef[], payloadTypes: PayloadType[], selectedUnitIds: string[] | null = null) {
    if (isRunning) return;
    ...
    testQueue = tasks.map(task => {
        ...
        const filteredPayloads = isCompression
            ? payloadTypes
            : (payloadTypes.length > 0 ? payloadTypes : ['text' as PayloadType]);
    ...
    testQueue.forEach(q => {
        ...
        q.payloadTypes.forEach(() => {
            benchmarks.forEach(b => {
                ...
                if (['sessionstorage', 'localstorage', 'store2'].includes(b.id) && q.task.sizeValue >= 4 * 1024 * 1024) return;
                totalUnits++;
            });
        });
    });
    ...
    callbacks?.onLog(`Benchmark session started with ${totalUnits} units.`, 'system', 'info');
