...
export async function runStorageLifecycle(
  ...
  name: string,
  iterations: number = 1,
  onProgress?: (step: string, iteration: number, duration: number) => void
  ...
  try {
    ...
    else if (!result.errors.update && result.update === 0)
      result.errors.update = 1;
    else if (!result.errors.delete && result.delete === 0)
      result.errors.delete = 1;
