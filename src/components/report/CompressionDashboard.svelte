<script lang="ts">
                                ...
                                if (context.parsed.y !== null) {
                                    ...
                                    if (isRatio) {
                                        ...
                                        if (compressedSize > 1024 * 1024) {
                                            ...
                                            sizeStr =
                                                (compressedSize / 1024).toFixed(
                                                    2,
                                                ) + " KB";
                                        } else {
                                            sizeStr =
                                                compressedSize.toFixed(0) +
                                                " B";
                                        }
                                        label += ` (${sizeStr})`;
                                    }
            ...
            metrics.forEach((m) => {
                ...
                techs.forEach((tech) => {
                    const points = actualSizes.map((s) => {
                        ...
                        const sizeData = (data as any)[s] || {};
                        Object.keys(sizeData).forEach((pt) => {
                            if (payloadFilter !== "all" && pt !== payloadFilter)
                                return;
                ...
                if (emptyEl && wrapEl && tableEl && toggleEl) {
                    ...
                    if (hasPoints) {
                        ...
                        chart.data.datasets.forEach((d) => {
                            const techName = (d.label || "").replace(
                                " (Collate)",
                                "",
                            );
                            ...
                            chart.data.datasets.forEach((d: any) => {
                                ...
                                if (val === null) {
                                    ...
                                    if (compSize > 1024 * 1024) {
                                        sizeStr =
                                            (compSize / 1024 / 1024).toFixed(
                                                2,
                                            ) + " MB";
                                    } else if (compSize > 1024) {
                                        sizeStr =
                                            (compSize / 1024).toFixed(2) +
                                            " KB";
                                    } else {
    ...
    async function handleRun(_type: "all" | "custom") {
        ...
        const selectedSizes = Object.keys(sizeChecks).filter(
            (k) => sizeChecks[k],
        );
        ...
        {#if !store.isRunning && !store.hasData}
            ...
            >
                <p style="margin-bottom: 1.5rem;">
                    No compression benchmark data yet.
                </p>
                <button
        ...
        <div style="display: {store.hasData ? 'block' : 'none'}; width: 100%;">
            <div class="filter-bar" bind:this={filterBarElement}>
                ...
                <div class="filter-chips">
                    <button
                        class="filter-chip {payloadFilter === 'all'
                            ? 'active'
                            : ''}"
                    ...
                    {#each ["text", "json", "random", "binary", "image", "pdf"] as p}
                        ...
                        {#if hasDataForType}
                            <button
                                class="filter-chip {payloadFilter === p
                                    ? 'active'
                                    : ''}"
            ...
            <div class="report-grid">
                <CompressionChartSection
                    title="Compression Performance Details"
                />
            </div>
        </div>
    </div>
    ...
    <div class="floating-filter-container">
        <div class="floating-filter-glass">
            ...
            <div class="floating-chips">
                <button
                    class="floating-chip {payloadFilter === 'all'
                        ? 'active'
                        : ''}"
                    onclick={() => (payloadFilter = "all")}
                >
                ...
                {#each ["text", "json", "random", "binary", "image", "pdf"] as p}
                    ...
                    {#if hasDataForType}
                        <button
                            class="floating-chip {payloadFilter === p
                                ? 'active'
                                : ''}"
                            onclick={() => (payloadFilter = p)}
                        >
