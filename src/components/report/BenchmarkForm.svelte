      ...
      <div class="matrix-row">
        ...
        <div class="row-ops-inline">
          ...
          <button
            class="btn-text-mini"
            onclick={() =>
              SIZE_METADATA.forEach((meta) => (sizeChecks[meta.id] = true))}
            >All</button
          >
          <button
            class="btn-text-mini"
            onclick={() =>
              SIZE_METADATA.forEach((meta) => (sizeChecks[meta.id] = false))}
            >None</button
          >
        </div>
      </div>
              ...
              {#each [["text", "Text"], ["json", "JSON"], ["random", "Text Random"], ["binary", "Binary"], ["image", "Image"], ["pdf", "PDF"]] as [val, label]}
                <label>
                  <input
                    type="checkbox"
                    class="payload-check"
                    bind:checked={payloadChecks[val as string]}
                    value={val}
                  />
                  {label}
                </label>
              ...
              onclick={() => {
                Object.keys(payloadChecks).forEach((k) => {
                  if (store.type === "storage") {
                    ...
                    payloadChecks[k] = ["text", "json", "image"].includes(k);
                  }
                });
              }}>Default</button
            >
