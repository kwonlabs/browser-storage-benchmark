<script lang="ts">
    ...
    function computeSummary(
                    ...
                    payloadTypes.forEach((pt) => {
                        ...
                        Object.keys(sizeData).forEach((tech) => {
                            ...
                            if (entry.insert > 0) {
                                ...
                                if (
                                    !skipList.includes(tech) &&
                                    entry.insert < bestWrite
                                ) {
        ...
        return {
            type,
            bestWrite:
                bestWriteTech !== "--" ? `${bestWrite.toFixed(2)}ms` : "--",
            bestWriteTech,
            bestRead: bestReadTech !== "--" ? `${bestRead.toFixed(2)}ms` : "--",
            bestReadTech,
            bestUpdate:
                bestUpdateTech !== "--" ? `${bestUpdate.toFixed(2)}ms` : "--",
            bestUpdateTech,
            bestDelete:
                bestDeleteTech !== "--" ? `${bestDelete.toFixed(2)}ms` : "--",
            bestDeleteTech,
            bestCompress:
                bestCompressTech !== "--"
                    ? `${bestCompress.toFixed(1)}MB/s`
                    : "--",
            bestCompressTech,
            bestDecompress:
                bestDecompressTech !== "--"
                    ? `${bestDecompress.toFixed(1)}MB/s`
                    : "--",
            bestDecompressTech,
            bestRatio:
                bestRatioTech !== "--" ? `${bestRatio.toFixed(1)}%` : "--",
            bestRatioTech,
