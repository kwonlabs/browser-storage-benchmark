  ...
  async function handleSessionClick(session: BenchmarkSession) {
    if (store.compareMode) {
      ...
      if (idx >= 0) {
        ...
        store.compareSelection = [...store.compareSelection, session.id];
      }
    } else {
    ...
    reader.onload = async (event) => {
      try {
        ...
        if (Array.isArray(imported)) {
          ...
          sessions = [...imported, ...sessions].sort((a, b) => b.id - a.id);
          store.addLog(
...
{#if store.historyOpen}
  <div class="history-panel" id="history-panel">
    <HistoryHeader {store} onExport={handleExport} onImport={handleImport} />
