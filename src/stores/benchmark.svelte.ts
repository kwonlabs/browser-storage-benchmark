import type { BenchmarkData, PayloadType, TaskDef } from '../lib/benchmarks/types';
...
class BenchmarkStore {
    ...
    progress = $state({ percent: 0, message: 'System initialized. Waiting for instructions...' });
    ...
    constructor(type: 'storage' | 'compression' = 'storage') {
        ...
        import('../lib/utils/ua').then(async ({ getFullEnvironmentInfo }) => {
            ...
            this.addLog('System initialized. Waiting for instructions...', 'system', 'info');
        });

        // Setup persistence effects
        $effect.root(() => {
            $effect(() => {
                localStorage.setItem(`console_visible_${this.type}`, String(this.consoleVisible));
            });
            $effect(() => {
                localStorage.setItem(`console_open_${this.type}`, String(this.consoleOpen));
            });
        });
    }
    ...
    get hasData(): boolean {
        return (
            Object.keys(this.data.low).some(s => Object.keys(this.data.low[s] ?? {}).some(p => Object.keys(this.data.low[s]?.[p] ?? {}).length > 0)) ||
            Object.keys(this.data.high).some(s => Object.keys(this.data.high[s] ?? {}).some(p => Object.keys(this.data.high[s]?.[p] ?? {}).length > 0)) ||
            Object.keys(this.data.compression).some(s => Object.keys(this.data.compression[s] ?? {}).some(p => Object.keys(this.data.compression[s]?.[p] ?? {}).length > 0))
        );
    }
    ...
    // Methods
    // ─────────────────────────────────────────
    addLog(message: string, type: 'system' | 'success' | 'error' = 'system', level: LogLevel = 'info') {
        const timestamp = new Date().toLocaleTimeString('en-US', { hour12: false, hour: '2-digit', minute: '2-digit', second: '2-digit', fractionalSecondDigits: 3 });
        this.logs = [...this.logs, { id: getLogId(), message, type, level, timestamp }];
    }
    ...
    startRun(tasks: TaskDef[], payloadTypes: PayloadType[], selectedUnitIds: string[] | null = null) {
        ...
        setRunnerCallbacks({
            ...
            onLog: (message, type: 'system' | 'success' | 'error' = 'system', level: LogLevel = 'info') => {
            ...
            onFinished: async (data: BenchmarkData) => {
                console.log(`[Store:${this.type}] Benchmark finished. Attempting to save...`, data);
                ...
                try {
                    ...
                    const id = await saveSession(this.type, JSON.parse(JSON.stringify(plainData)), plainLogs, { os: info.os, browser: info.browser });
                    console.log(`[Store:${this.type}] Save success, ID:`, id);
                    this.addLog(`Benchmark results saved to ${this.type} history (ID: ${id}).`, "success");
                } catch (e) {
                    console.error(`[Store:${this.type}] Final save failure:`, e);
                    this.addLog("Failed to save session to history (Serialization Error).", "error");
                }
            },
            onCancelled: () => {
                this.isRunning = false;
        ...
        try {
            ...
            if (sessions && sessions.length > 0) {
                ...
                if (latest) {
                    ...
                    this.addLog(`Automatically loaded most recent ${this.type} benchmark session (#${latest.sessionId}).`, 'system', 'info');
                    return true;
                }
            }
        } catch (e) {
